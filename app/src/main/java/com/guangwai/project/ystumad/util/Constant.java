package com.guangwai.project.ystumad.util;

/**
 * 统一存放常量的工具类
 * Created by Ming on 2018/3/16.
 */

public class Constant {
    public static final String[] TITILES = {"练习模式", "闯关模式"};

    public static final int TAB1_INDEX = 1;
    public static final int TAB2_INDEX = 2;
    public static final int TAB3_INDEX = 3;

    public static final int PRATICE_MODE = 0;
    public static final int BREAK_MODE = 1;

    public static final int SINGLE_MODE = 0;
    public static final int MIXED_MODE = 1;

    public static final int OPERATION_ADD = 0; //加法
    public static final int OPERATION_REDUCE = 1; //减法
    public static final int OPERATION_MULTIPLY = 2; //乘法
    public static final int OPERATION_DIVIDE = 3; //除法

    public static final int LAST_ONE = 0; //上一题
    public static final int NEXT_ONE = 1; //下一题

    public static final int ASR_END = 3; //语音结束


}
